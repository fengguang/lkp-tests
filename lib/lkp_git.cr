...
def git_commit(commit)
  ...
  $__git_commit_cache ||= {}
  return $__git_commit_cache[commit] if $__git_commit_cache.includes?(commit)
...
def __commit_tag(commit)
  ...
  tags.each do |tag|
    return tag if linus_tags.includes? tag
  end
  tags[0]
end
...
def commit_tag(commit)
  $__commit_tag_cache ||= {}
  return $__commit_tag_cache[commit] if $__commit_tag_cache.includes?(commit)
