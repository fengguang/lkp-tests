...
additional_perf_metrics_prefixes = $test_prefixes.reject do |test|
  test_name = test[0..-2]
  functional_test?(test_name) || other_test?(test_name) || %w(kmsg dmesg stderr last_state).includes?(test_name)
end
...
def perf_metric?(name)
  ...
  if $__is_perf_metric_cache.includes? name
...
def stat_relevance(record)
  ...
  relevance = if stat[0..9] == "lock_stat."
                5
              elsif $test_prefixes.includes? stat.sub(/\..*/, ".")
                100
...
def is_failure(stats_field)
  $__is_failure_cache ||= {}
  if $__is_failure_cache.includes? stats_field
...
def is_latency(stats_field)
  $__is_latency_cache ||= {}
  if $__is_latency_cache.includes? stats_field
...
def __get_changed_stats(a, b, is_incomplete_run, options)
  ...
  b.keys.each { |k| a[k] = [0] * cols_a unless a.includes?(k) }
...
def kpi_stat?(stat, _axes, _values = nil)
  return false if $kpi_stat_blacklist.includes?(stat)

  base, _, remainder = stat.partition(".")
  all_tests_set.includes?(base) && !remainder.start_with?("time.")
end
